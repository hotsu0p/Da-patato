...
void main() {
	...
	gl_FragData[0] = texture2D(texture, lmtexcoord.xy);
		...
		vec3 ambient = (-NdotL*0.45+0.9)*lightCol.rgb + skyIntensity*vec3(0.5,0.65,1.)*40. + skyIntensityNight*vec3(0.09,0.1,0.15)/1.5;
	
		vec3 diffuseLight = lmtexcoord.w*ambient*0.1 + torch_lightmap*0.2*vec3(1.,0.42,0.11) + 0.02;

		vec3 col = dot(diffuseLight,vec3(1.0/3))*albedo;
		
			
		gl_FragData[0].rgb = col*10.;
