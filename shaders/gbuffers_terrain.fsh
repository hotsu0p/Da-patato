...
void main() {
	...
	bool translucent = normalMat.w<0.01;
	...
	if (gl_FragData[0].a  > 0.0) {
		...
		if (translucent){
			diffuseSun = diffuseSun+(0.5+abs(NdotL)*pow(clamp(lightCol.a*dot(normalize(fragpos),sunVec),0.0f,1.0f),10.)*2.);
		}
		...
		if (diffuseSun > 0.001) {
			...
			if (abs(projectedShadowPosition.x) < 1.0-1.5/shadowMapResolution && abs(projectedShadowPosition.y) < 1.0-1.5/shadowMapResolution){
				...
				if (!translucent) diffthresh = (facos(diffuseSun)*0.008*sr+0.00008*sr)/(distortFactor*distortFactor);
				//smooth out depth threshold
				diffthresh=mix(diffthresh,diffthresh*0.75,interleaved_gradientNoise());
				projectedShadowPosition = projectedShadowPosition * vec3(0.5,0.5,0.5/3.0) + vec3(0.5,0.5,0.5);
					...
					float shading = dot(vec4(shadow2D(shadow,vec3(projectedShadowPosition.st + vec2(offset,offset), projectedShadowPosition.z)).x,shadow2D(shadow,vec3(projectedShadowPosition.st + vec2(-offset,offset), projectedShadowPosition.z)).x,shadow2D(shadow,vec3(projectedShadowPosition.st + vec2(offset,-offset), projectedShadowPosition.z)).x,shadow2D(shadow,vec3(projectedShadowPosition.st + vec2(-offset,-offset), projectedShadowPosition.z)).x),vec4(0.25));
		...
		vec3 diffuseLight = direct + lightmap*color.a;
